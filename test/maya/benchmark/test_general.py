...
import random
import time
import mayarv.maya.nodes.iter as iter
...
class TestGeneralPerformance( unittest.TestCase ):
	...
	def test_dagwalking( self ):
		...
		for nodecount in numnodes:
			...
			# NO NODE CONVERSION
			starttime = time.time( )
			for dagpath in iter.iterDagNodes( dagpath = 1 ):
				pass
			elapsed = time.time() - starttime
			...
			for node in iter.iterSelectionList( sellist, handlePlugs = False, asNode = False ):
				pass
			elapsed = time.time() - starttime
			...
			starttime = time.time( )
			for node in iter.iterDagNodes( asNode = 1 ):
				pass
			elapsed = time.time() - starttime
			...
			# BREADTH
			starttime = time.time( )
			for dagpath in iter.iterDagNodes( depth = 0 ):
				pass
			elapsed = time.time() - starttime
